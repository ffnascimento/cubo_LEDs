...
// Please UNCOMMENT one of the contructor lines below //painel LCD      
// U8x8 Contructor List //painel LCD      
// The complete list is available here: https://github.com/olikraus/u8g2/wiki/u8x8setupcpp //painel LCD      
// Please update the pin numbers according to your setup. Use U8X8_PIN_NONE if the reset pin is not connected //painel LCD      
...
//U8X8_SSD1306_128X64_NONAME_SW_I2C u8x8(/* clock=*/ 2, /* data=*/ 0, /* reset=*/ U8X8_PIN_NONE); 	      // Digispark ATTiny85 //painel LCD      
//U8X8_SSD1306_128X64_NONAME_SW_I2C u8x8(/* clock=*/ SCL, /* data=*/ SDA, /* reset=*/ U8X8_PIN_NONE);   // OLEDs without Reset of the Display //painel LCD      

// End of constructor list //painel LCD      
...
void setup(void)
{
  ...
  vw_set_rx_pin(5); // Define o pino do Arduino a ser entrada de dados do receptor
  vw_set_ptt_inverted(true); // necessário para nosso receptor
  vw_setup(2000);             // taxa de transferência a ser recebida (em bps)
  vw_rx_start();              // Inicializa o receptor

  u8x8.drawString(0,0,"Aguardando...");//painel LCD
  u8x8.setInverseFont(0);//painel LCD

  delay(2000); //delay de inicialização
...
void loop(void)
{
  ...
  memset(message, 0, VW_MAX_MESSAGE_LEN);  // limpa o buffer antes de receber. Evita uso de lixo
